...
// globalMemPtr is an array of 8 bytes allocated in global memory.
...
int main()
{
    // Two integers are constructed within global memory(globalMemPtr[8]) using placement new.
    // when used with "placement new," you can specify a memory location for the object to be constructed.
    // (void*) is a type cast: sing void* indicates that the memory being pointed to is untyped, meaning it is
	// treated as a sequence of raw bytes without a specific type.
	// int(5): This part of the expression initializes an int object with the value 5.
   auto objPtrOnData1 = new((void*)globalMemPtr) int(5);
   auto objPtrOnData2 = new((void*)globalMemPtr+4) int(6);

   // stackMemPtr is an array of 8 bytes allocated in stack memory.
   ...
   // An array of 10 bytes is allocated in heap memory (heapMemPtr).
   ...
   // Test
   //An allocator for the custom class B is instantiated
   allocator<B> allocator1;
   // reserves a block of memory :  for 10 objects of type B.
